...
class Slidey {
  static double buttonHeight =
      90; // Height of each button for easy changing access
      ...
      builder: (BuildContext context) => ListView(
              ...
              children: <Widget>[
                ...
                _buttonRow([
                  _snackButton(
                      context,
                      const Icon(CustomIcons.userTimes, color: Colors.amber),
                      "PowerSchool"), // Button1 = PowerSchool
                  _snackButton(
                      context,
                      const Icon(CustomIcons.wallet, color: Colors.amber),
                      "Naviance"), // Button2 = Naviance
                  _linkButton(
                      context,
                      const Icon(Icons.contact_mail, color: Colors.amber),
                      "https://mail.ucvts.tec.nj.us/",
                      "the Outlook District Email") // Button3 = Outlook
                ]),

                _buttonRow([
                  _linkButton(
                      context,
                      const Text("School Lunch"),
                      "https://ucvts.nutrislice.com/menu/uvcts-cafeteria/lunch/${DateTime.now().year}-${DateTime.now().month}-${DateTime.now().day}",
                      "today's lunch menu"),
                  _pageButton(context, const Text("Staff"),
                      const StaffDirectoryPage()) // "Open a seperate flutter page with Staff Directory"
                ]),

                _buttonRow([
                  // Third row of buttons
                  _pageButton(context, const Text("District News Letter"),
                      DistrictNewsLetterPage()) // "Built in PDF -> Talk to Kneisel"
                ]),

                _buttonRow([
                  // Fourth row of buttons
                  _pageButton(context, const Text("Important Forms"),
                      const ImportantFormsPage()) // "Flutter page with direct links to PDFs"
                ])
              ]));
...
Widget _snackButton(BuildContext context, Widget inside, String text) {
  // context BuildContext to build the SnackBar
  // inside Widget to show what the button displays to the user
  // text String to provide text for SnackBar display
...
Widget _linkButton(
    BuildContext context, Widget inside, String link, String errorMessage) {
  return ElevatedButton(
      style: ElevatedButton.styleFrom(
        minimumSize: Size.fromHeight(Slidey
            .buttonHeight), // We need this stupid style thing because I think the ListView and/or the panel itself is fucking with the normal ButtonBar height setter...
